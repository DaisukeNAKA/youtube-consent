...
async function loadAll() {
  ...
  // バストステッカー（水平カット）: 中塚は写真y=1400（ハートの手の上端y≈1440より上）、
...
const BUST_CUT = { nakatsuka: (1400 - 224) / 2352, ricchan: 0.68, ricchanShort: (1530 - 725) / 1851 };   // レイヤー高さに対する水平カット位置
...
function bgSky(t) {
  ...
  g.addColorStop(0, '#16C8F0'); g.addColorStop(.42, '#BDF4FF'); g.addColorStop(.7, '#A8F28A'); g.addColorStop(1, '#46D83A');
  ctx.fillStyle = g; ctx.fillRect(0, 0, W, H);
  // ふわふわ雲（ロゴのエアブラシ感）
  const r = rngFactory(11);
  for (let i = 0; i < 9; i++) {
    const cx = ((r() * 1.4 - .2) * W + t * 18 * U * (0.5 + r())) % (W * 1.3) - W * .15, cy = r() * H * .55, rad = (180 + r() * 260) * U;
...
function person(key, x, y, h, { ax = .5, ay = 1, rot = 0, alpha = 1, shadow = true, sticker = true, flip = false, clip = null } = {}) {
...
function text(str, x, y, o = {}) {
  ...
  { const sw = Math.max(0, ...strokes.map(s => s[1])) * size; const hh = size * (o.latin ? .6 : .52); recBox(o.qa || 'telop', str, [[cx - sw, -hh - sw], [cx + tw + sw, -hh - sw], [cx - sw, hh + sw], [cx + tw + sw, hh + sw]]); }
...
const SAFE = V
  ...
  : { rects: [[96, 183, 1758, 693], [496, 38, 1445, 183]] };
...
// 吹き出し付きテロップ（話者色プレート）。tail: 顔の方向 {x,y}
const PLATE = {
  nakatsuka: { bg: '#7C4DFF', fg: '#FFFFFF', edge: '#1B1033' },   // 紫地×白字 4.8:1
  ricchan: { bg: '#FF4FA3', fg: '#1B1033', edge: '#FFFFFF' },     // ピンク地×インク字 5.9:1
  both: { bg: '#FFFFFF', fg: '#1B1033', edge: '#1B1033' },
};
function sayBox(spk, lines, x, y, t, t0, o = {}) {
  ...
  const lhs = sizes.map(sz => sz * (lines.length > 1 ? 1.22 : 1.1));
  ...
  const tails = o.tails || (o.tail ? [o.tail] : []);
  withT(() => {
    const L = -w / 2, T0 = -h / 2, r = Math.min(34 * U, h / 2);
    ctx.fillStyle = 'rgba(27,16,51,.32)'; rrect(L + 8 * U, T0 + 12 * U, w, h, r); ctx.fill();
    ctx.beginPath(); rrect(L, T0, w, h, r);
    for (const tl of tails) {   // しっぽ: 話者の方向を示すだけ（最大70px、頭部の手前で止める）
      const kk = k * (o.scale || 1), tx = (tl.x - x) / kk, ty = (tl.y - y) / kk;
      const bx = clamp(tx, L + r + 30 * U, L + w - r - 30 * U), by = ty > 0 ? T0 + h : T0;
      const dx = clamp((tx - bx) * .55, -70 * U, 70 * U);
      ctx.moveTo(bx - 24 * U, by); ctx.lineTo(bx + dx, by + (ty > 0 ? 1 : -1) * Math.min(Math.abs(ty - by), (tl.len || 60) * U)); ctx.lineTo(bx + 24 * U, by);
    }
    ctx.fillStyle = P.bg; ctx.strokeStyle = P.edge; ctx.lineWidth = 7 * U; ctx.lineJoin = 'round'; ctx.stroke(); ctx.fill();
    ...
    lines.forEach((ln, i) => {
      ...
      const d = o.lineDelay || 0;
      const ki = d ? lerp(.6, 1, E.outBack(prog(t, t0 + i * d, t0 + i * d + .2), 1.4)) : 1;
    ...
    recBox(o.qa || 'telop', lines.join(''), [[L, T0], [L + w, T0], [L, T0 + h], [L + w, T0 + h]], { spk });
...
function head(key, box, o = {}) {
  ...
  ctx.save(); ctx.translate((x0 + x1) / 2 + (o.dx || 0), (y0 + y1) / 2 + (o.dy || 0)); if (o.rot) ctx.rotate(o.rot); if (o.sx || o.sy) ctx.scale(o.sx || 1, o.sy || 1);
  recBox('head', who, [[-bw / 2, -bh / 2], [bw / 2, -bh / 2], [-bw / 2, bh / 2], [bw / 2, bh / 2]], { h: bh });
  person(key, 0, 0, h, { ax, ay, alpha: o.alpha ?? 1 });
  ctx.restore();
}
// 頭部枠の中の「顔中心」（拡大の基準点）
...
function pill(label, x, y, size, { bg = C.white, edge = C.pink, fg = C.ink, font = F.round, weight = '900 ', tri = false, r = null, qa = 'label', k = 1, rot = 0, padX = 34 } = {}) {
  if (k <= 0) return;
  withT(() => {
    ...
    recBox(qa, label, [[-w / 2, -h / 2], [w / 2, -h / 2], [-w / 2, h / 2], [w / 2, h / 2]]);
  }, x, y, k, rot);
}
