...
// usage: node tools/render.mjs <916|169> <out.mp4> [audio.wav]
...
await page.goto(`http://127.0.0.1:${port}/cm.html?fmt=${fmt}&export=1`);
...
if (only) {
  ...
  args.push('-vf', 'scale=out_color_matrix=bt709:out_range=tv,format=yuv420p',
    '-c:v', 'libx264', '-preset', 'slow', '-crf', '16', '-profile:v', 'high', '-level', fmt === '169' ? '4.1' : '4.2',
    '-colorspace', 'bt709', '-color_primaries', 'bt709', '-color_trc', 'bt709', '-color_range', 'tv',
    '-r', '30', '-g', '30', '-movflags', '+faststart', '-t', '15', out);
  const ff = spawn(FFMPEG, args, { stdio: ['pipe', 'inherit', 'inherit'] });
  const t0 = Date.now(); const boxes = [];
  for (let i = 0; i < N; i++) {
    const url = await page.evaluate(i => window.__frame(i), i);
    const buf = Buffer.from(url.split(',')[1], 'base64');
    boxes.push(await page.evaluate(() => window.__boxes));
  ...
  fs.writeFileSync(out.replace(/\.mp4$/, '') + '.qa_boxes.json', JSON.stringify({ fmt, W, H, fps: 30, frames: boxes }));
